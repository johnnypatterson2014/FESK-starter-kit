...
import com.calamansi.demo.login.CustomAuthorizedClientService;
...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.csrf(AbstractHttpConfigurer::disable)
				...
				.authorizeHttpRequests(
						(auth) -> auth.requestMatchers("/secure/**").authenticated().anyRequest().permitAll()

				)
				// Disable "JSESSIONID" cookies
				.sessionManagement(manager -> manager.sessionCreationPolicy(SessionCreationPolicy.STATELESS))

				.oauth2Login(config -> config.authorizedClientService(this.customAuthorizedClientService)
	...
	public UrlBasedCorsConfigurationSource corsConfigurationSource() {
		...
		configuration.setAllowedOrigins(Arrays.asList("http://localhost:3300", "http://127.0.0.1:3300"));
